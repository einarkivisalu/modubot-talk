...
def format_example(ex):
    ...
    # Prefer tokenizer helper if present
    if hasattr(tokenizer, "apply_chat_template"):
        try:
            ...
            text = tokenizer.apply_chat_template(
                messages,
                tokenize=False,
                add_generation_prompt=True   # changed from False to True
            )
    ...
    # Fallback format with explicit assistant marker
    text = (
        f"SYSTEM: {system_msg}\n\n"
        f"USER: {user_msg}\n\n"
        f"ASSISTANT: {assistant_msg}"   # explicit marker
    )
